...
const content = {
  ...
  ingredientPhoto: pathToFileURL(path.join(__dirname, 'assets', 'ingredientes-salmon-cordero-horizontal.png')).href,
...
const pageUrl = `${pathToFileURL(path.join(__dirname, 'visual-explicito.html')).href}?content=${encodeURIComponent(encoded)}`;
const output = path.join(__dirname, '02-ingredientes-visuales-salmon-cordero-1080x1350.png');
