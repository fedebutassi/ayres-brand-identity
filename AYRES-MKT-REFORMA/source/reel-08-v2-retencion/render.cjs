...
// Reutiliza el template HTML/CSS/JS del lote 01
const source = path.join(repoRoot, 'AYRES-MKT-REFORMA', 'source', 'reels-lote-01', 'reel.html');
const outputRoot = path.join(repoRoot, 'AYRES-MKT-REFORMA', 'produccion', 'reels', 'reel-08-v2-retencion');
...
for (const reel of reels) {
  ...
  const motionDir = path.join(outputRoot, 'motion');
