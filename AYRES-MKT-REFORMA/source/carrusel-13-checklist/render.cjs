...
const outputRoot = path.join(repoRoot, 'AYRES-MKT-REFORMA', 'produccion', 'carruseles', '13-checklist-comparar-alimentos');
...
const hydrateProduct = (item) => ({ ...item, image: imageFor(item.id) });
const payload = {
  ...content,
  ...
  slides: content.slides.map((slide, index) => ({
    ...
    products: slide.products?.map(hydrateProduct),
    comparison: slide.comparison?.map((item) => ({ ...item, image: imageFor(item.imageId) })),
  })),
};
...
const render = (index) => new Promise((resolve, reject) => {
  ...
  const slug = ['portada', 'etapa-de-vida', 'tamano-o-segmento', 'composicion-centesimal', 'primeros-ingredientes', 'presentaciones', 'cta-checklist'][index];
  const output = path.join(outputRoot, `${number}-${slug}-1080x1350.png`);
